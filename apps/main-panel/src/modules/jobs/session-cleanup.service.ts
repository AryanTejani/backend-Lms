...
export class SessionCleanupService implements OnModuleInit {
  ...
  private async runCleanupWithLogging(): Promise<void> {
    ...
    try {
      ...
      if (total > 0) {
        this.logger.log(`Session cleanup completed: ${result.deletedRevoked} revoked, ${result.deletedExpired} expired`);
      }
    } catch (error) {
  ...
  async cleanupSessions(): Promise<{ deletedRevoked: number; deletedExpired: number }> {
    ...
    const deletedRevoked = await this.sessionRepository.cleanupRevoked(retentionDays, BATCH_SIZE);

    // Delete expired sessions (past max age, even if not explicitly revoked)
    const deletedExpired = await this.sessionRepository.cleanupExpired(maxAgeDays, BATCH_SIZE);
