...
import { Session, SessionWithCustomer } from '../types/auth.types';
...
export class SessionRepository {
  ...
  async findActiveWithCustomer(sessionId: string): Promise<SessionWithCustomer | null> {
    ...
    >`
      ...
      WHERE s.id = ${sessionId}::uuid AND s.revoked_at IS NULL
        AND s.created_at > NOW() - INTERVAL '1 day' * ${this.configService.get<number>('session.maxAgeDays') ?? 30}
    `;
   ...
   */
  private mapToSession(session: { id: string; customerId: string; createdAt: Date; revokedAt: Date | null }): Session {
    return {
      id: session.id,
