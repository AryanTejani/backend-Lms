...
import { generateOAuthState, generateCodeVerifier, generateCodeChallenge } from '@app/shared/utils/crypto.util';
import { Errors } from '@app/shared/exceptions/auth.exception';
import { OAuthUrl, OAuthCallbackParams, GoogleTokenResponse, GoogleUserInfo, AuthenticatedUser, Session } from '../types/auth.types';
...
export class OAuthService {
  ...
  async getGoogleAuthUrl(): Promise<OAuthUrl> {
    ...
    await this.cacheService.createOAuthState(state, 'google', codeVerifier, redirectUri);
  ...
  async handleGoogleCallback(params: OAuthCallbackParams): Promise<{ user: AuthenticatedUser; session: Session; isNewUser: boolean }> {
    ...
    if (!tokenResponse.ok) {
      ...
      this.logger.error(`OAuth token exchange failed: ${errorBody}`);
      throw Errors.oauthProviderError('Google', 'Token exchange failed');
    }
