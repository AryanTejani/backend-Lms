...
export class CacheService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to connect to Redis at ${redisUrl.replace(/\/\/.*@/, '//<credentials>@')}: ${message}`,
      );
      throw error;
    }
  }
  ...
  async cacheSession(
    ...
    const key = `${SESSION_CACHE_PREFIX}${sessionId}`;
    const cacheTtl =
      this.configService.get<number>('session.cacheTtlSeconds') ?? 300;
  ...
  async setPasswordResetCooldown(email: string): Promise<void> {
    ...
    const cooldownSeconds =
      this.configService.get<number>('passwordReset.cooldownSeconds') ?? 60;
  ...
  async storePasswordResetToken(
    ...
    const tokenTtlMinutes =
      this.configService.get<number>('passwordReset.tokenTtlMinutes') ?? 60;
